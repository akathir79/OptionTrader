...
class ColumnVisibilityController {
    ...
    // New method to rebuild table with only visible columns
    rebuildTableForVisibleColumns() {
        const table = document.getElementById('optionChainTable');
        if (!table) return;
        ...
        // Rebuild header with only visible columns
        const visibleHeaders = [];
        this.columnNames.forEach((name, index) => {
            if (this.columnStates[index]) {
                visibleHeaders.push(`<th>${name}</th>`);
            }
        });
        thead.innerHTML = visibleHeaders.join('');

        // For now, clear tbody - data will be regenerated by WebSocket handler
        tbody.innerHTML = '';
        
        console.log('Table structure rebuilt for visible columns');
    }
    ...
    refreshTableData() {
        ...
        console.log('Refreshing table structure and data due to column visibility change');
        
        // First rebuild the table structure with only visible columns
        this.rebuildTableForVisibleColumns();
        
        // Then refresh the data if WebSocket handler exists
        if (window.webSocketHandler && typeof window.webSocketHandler.refreshOptionChain === 'function') {
            // Add a small delay to ensure the table structure is rebuilt first
            setTimeout(() => {
