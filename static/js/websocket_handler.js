...
class WebSocketHandler {
    ...
    /**
     * Get option chain strikes data for payoff chart
     * @returns {Array|null} Array of strike data or null if not available
     */
    getOptionChainStrikes() {
        // Try to get from stored option chain data
        if (this.lastOptionChainData && this.lastOptionChainData.strikes) {
            const strikes = this.lastOptionChainData.strikes;
            console.log(`📊 Returning ${strikes.length} strikes from stored option chain data`);
            return strikes;
        }
        
        // Extract from option chain table as fallback
        const optionTable = document.getElementById('optionChainTable');
        if (!optionTable) {
            console.log('⚠️ No option chain table found');
            return null;
        }
        ...
        rows.forEach(row => {
            ...
            if (strikeCell) {
                ...
                if (!isNaN(strike)) {
                    strikes.push({ 
                        strike: strike,
                        ce_ltp: this.getCellValue(row, 'ce-ltp'),
                        pe_ltp: this.getCellValue(row, 'pe-ltp')
                    });
                }
            }
        });
    ...
    /**
     * Get cell value from option chain table row
     * @param {Element} row - Table row element
     * @param {string} cellClass - CSS class of the cell to extract
     * @returns {number} Cell value as number
     */
    getCellValue(row, cellClass) {
    ...
    /**
     * Store option chain data for later access
     * @param {Object} data - Option chain data from API
     */
    storeOptionChainData(data) {
        this.lastOptionChainData = data;
        console.log(`💾 Stored option chain data with ${data.strikes ? data.strikes.length : 0} strikes`);
    }
}
